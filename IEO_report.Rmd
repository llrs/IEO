...
```{r de.number}
de_tumor <- subset(tt_tumor, abs(logFC) > 2 & B > 4.6 & adj.P.Val < 0.05)
de_gender <- subset(tt_gender, abs(logFC) > 2 & B > 4.6 & adj.P.Val < 0.05)
de_males <- subset(tt_males, abs(logFC) > 2 & B > 4.6 & adj.P.Val < 0.05)
de_females <- subset(tt_females, abs(logFC) > 2 & B > 4.6 & adj.P.Val < 0.05)
nrow(de_tumor)
nrow(de_gender)
nrow(de_males)
nrow(de_females)
...
However we can extract the names of the other comparisons to see how much they overlap:
...
params_female_exc <- new("GOHyperGParams", geneIds = DE_females_exc,
                    universeGeneIds = unique(all.Human.GO$ENTREZID),
                    annotation = "org.Hs.eg.db", ontology = "BP",
                    pvalueCutoff = 0.05, testDirection = "over", 
                    conditional = TRUE)
...
hgOver_female_exc <- hyperGTest(params_female_exc)
hgOver_female_exc

GO_female_exc <- GOstats::summary(hgOver_female_exc)

GO_female_exc <- subset(GO_female_exc, Size >= 1 & Count >= 1 & Pvalue <= 0.05)

dim(GO_female_exc)

ggplot(GO_female_exc, aes(Size, Count, size = OddsRatio, colour = Pvalue)) + 
    geom_point() + b

GO_female_exc <- GO_female_exc[order(GO_female_exc$OddsRatio, decreasing = TRUE), ]
head(GO_female_exc)

geneIDs <- geneIdsByCategory(hgOver_female_exc)[GO_female_exc$GOBPID]
geneSYMs <- sapply(geneIDs, function(id) select(org.Hs.eg.db,
                    columns = "SYMBOL", key = id, keytype = "ENTREZID")$SYMBOL)
geneSYMs <- sapply(geneSYMs, paste, collapse = ", ")
GO_female_exc <- cbind(GO_female_exc, Genes = geneSYMs)
rownames(GO_female_exc) <- 1:nrow(GO_female_exc)
```
