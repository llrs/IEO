...
As we can see, in many variables the main value is just `NA` or other derivative which means "No information available for this variable". So in order to keep the most informative variables, we decided to apply a threshold of 60% (of the values containing informative data) to select the variables:
...
In the "normal" type samples most of the information is not avalable. This could be due to that all "normal" samples were obtained from patients with tumors. So in order to have the maximum information of each sample, we infered "normal" samples information from its paired sample data (mainly gender and ethnicity).
