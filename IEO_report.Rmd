...
After the looking for the batch effect we couldn't find clearly the cause of the batch effect. We suspect that sample vial has something to do. However, we try to see if we can improve the clustering by using method ComBat for sample vial. 
...
library("limma")
form <- as.formula("~ gender + type") # Building the model
mod  <- model.matrix(form, data = colData(thca.filtered))
qrexp <- removeBatchEffect(assays(thca.filtered)$cqn.logCPM, tss, design = mod)
...
sv <- sva(assays(thca.filtered)$qrexp, mod, mod0)
sv$n
```
...
```
After all this first analysis, we can conclude that aproximatelly 5000 genes have different expression levels among tumor and normal samples. 
...
```{r DE, cache = TRUE}
# Transform the log2 normalized by cqn to counts
cqn.n <- apply(assays(thca.filtered)$qrexp, 1:2, function(x){2^(x)}) 
# Normalize by mean variance with voom
vo <- voom(cqn.n, mod, plot = TRUE)
corfit <- duplicateCorrelation(vo, mod, block = colData(thca.filtered)$bcr_patient_barcode)
fit <- lmFit(vo, mod, block = colData(thca.filtered)$bcr_patient_barcode,
...
List of differentially expressed genes annotated with symbol and ensembl id:

```{r tt, cache = TRUE}
tt_tumor <- topTable(fit, number = Inf, coef = "typetumor")
tt_gender <- topTable(fit, number = Inf, coef = "genderMALE")
...
qqt(fit$t[, 2], df = fit$df.prior + fit$df.residual[2], main = "", pch = ".", cex = 3)
qqline(fit$t[, 2], lwd = 2, col = "red")
abline(0, 1, lwd = 2)
mtext("Comparing genes between gender", outer = TRUE, cex = 1.5, side = 3, line = -2)
hist(tt_tumor$P.Value, xlab = "Raw P-values", main = "", las = 1)
qqt(fit$t[, 3], df = fit$df.prior + fit$df.residual[3], main = "", pch = ".", cex = 3)
qqline(fit$t[, 3], lwd = 2, col = "red")
abline(0, 1, lwd = 2)
...
volcanoplot(fit, coef = 2, highlight = 7, fit$genes$symbol, main = "model Gender", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)
volcanoplot(fit, coef = 3, highlight = 7, fit$genes$symbol, main = "model Disease", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)
```
...
```{r de.number}
dim(tt_tumor[abs(tt_tumor$logFC) > 2 & tt_tumor$B > 4.6,])[1]
dim(tt_gender[abs(tt_gender$logFC) > 2 & tt_gender$B > 4.6,])[1]
```

## Differential expression of patients

Using the expression values with batch effect removed by qr decomposition we find the top differentially expressed genes:

```{r DE_tumors, cache = TRUE}
mod_tumors <- mod[1:22,]
# Normalize by mean variance with voom
vo <- voom(cqn.n[, rownames(mod_tumors)], mod_tumors, plot = TRUE)
fit <- lmFit(vo, mod_tumors)
fit <- eBayes(fit)
res <- decideTests(fit)
summary(res)
```
...
DE_tumor <- unique(tt_tumor[abs(tt_tumor$logFC) > 2 & 
                                  tt_tumor$B > 4.6 & 
                                  tt_tumor$adj.P.Val < 0.05, "Row.names"])

DE_gender <- unique(tt_gender[abs(tt_gender$logFC) > 2 & 
                                  tt_gender$B > 4.6 & 
                                  tt_gender$adj.P.Val < 0.05, "Row.names"])

params_tumor <- new("GOHyperGParams", geneIds = DE_tumor,
                    universeGeneIds = unique(all.Human.GO$ENTREZID),
                    annotation = "org.Hs.eg.db", ontology = "BP",
                    pvalueCutoff = 0.05, testDirection = "over", 
                    conditional = TRUE)

params_gender <- new("GOHyperGParams", geneIds = DE_tumor,
                    universeGeneIds = unique(all.Human.GO$ENTREZID),
                    annotation = "org.Hs.eg.db", ontology = "BP",
                    pvalueCutoff = 0.05, testDirection = "over", 
                    conditional = TRUE)
...
```{r fun.enrich.tumor, cache = TRUE}
hgOver_tumor <- hyperGTest(params_tumor)
hgOver_tumor
...
GO_tumor <- GO_tumor[GO_tumor$Size >= 10 & GO_tumor$Count >= 10, ]
...
```{r fun.enrich.gender, eval=FALSE, include=FALSE}
hgOver_gender <- hyperGTest(params_gender)
hgOver_gender

summary <- GOstats::summary
GO_gender <- summary(hgOver_tumor)

GO_gender <- GO_gender[GO_gender$Size >= 5 & GO_gender$Count >= 5, ]
GO_gender <- GO_gender[order(GO_gender$OddsRatio, decreasing = TRUE), ]
head(GO_gender)

geneIDs <- geneIdsByCategory(hgOver_gender)[GO_gender$GOBPID]
geneSYMs <- sapply(geneIDs, function(id) select(org.Hs.eg.db, 
            columns = "SYMBOL", key = id, keytype = "ENTREZID")$SYMBOL)
geneSYMs <- sapply(geneSYMs, paste, collapse=", ")
GO_gender <- cbind(GO_gender, Genes = geneSYMs)
rownames(GO_gender) <- 1:nrow(GO_gender)

```

Looking into the table:

```{r fun.enrich.visualize}
library(xtable)
...
# 
# xtab_gender <- xtable(GO_gender, align="l|c|r|r|r|r|r|p{3cm}|p{3cm}|")
# print(xtab_gender, file="GO_gender.html", type="html")
# 
# browseURL("GO_gender.html")
```
