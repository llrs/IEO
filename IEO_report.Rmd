...
qqnorm(sampledepth)
qqline(sampledepth)
# Looking into the qqplot we decide to filter out those below 40 or above 70.
...
avgexp <- rowMeans(assays(thca.filtered)$cqn.logCPM)
hist(avgexp, xlab = "log2 CPM", main = "", las = 1)
abline(v = 1, col = "red", lwd = 2)
```
...
Since we will use the offsets computed by `cqn`, there is no need to normalize using the normalization tools from `edgeR`, such as `calcNormFactors`.
