...
colnames(modsv) <- c(colnames(design), make.names(1:ncol(sv$sv)))
...
As we can see the distribution is now uniform, thus `sva` corrected for artifacts.
...
```{r remove_surrogates, eval=FALSE, autodep=TRUE}
...
Using the expression values with batch effect removed by `ComBat` and without the surrogate variables, we proceed with the voom normalization
...
```{r accuracy4, eval=FALSE, fig.cap="\\label{accuracy4}Figure 22.D: P-value distribution and comparison with the t-student distribution in the male comparison tumoral and healthy samples.", autodep=TRUE, include=FALSE}
pars <- par(mfrow = c(1, 2), mar = c(4, 5, 2, 2))
...
```{r volcano3, eval=FALSE, fig.cap="\\label{volcano3}Figure 23.C: Volcano plot of the comparison healthy males vs tumoral males.", autodep=TRUE}
...
de_genes <- c(nrow(de_tumor), nrow(de_gender), nrow(de_females))
names(de_genes) <- c("Tumor vs Normal", "Females vs Males", "Females")
de_genes
```
...
```{r venn2, fig.cap="\\label{venn2}Figure 24.B: Representation of the number of genes differentially expressed in comparison between males, and healthy vs tumoral.", autodep=TRUE, eval=FALSE}
plot(Venn(list(DE_tumor, DE_males),
              SetNames = c("Tumoral", "Males")))
```

```{r venn3, fig.cap="\\label{venn3}Figure 24.C: Representation of the number of genes differentially expressed in comparison between males and females.", autodep=TRUE, eval = FALSE}
