...
```{r plots1A, fig.cap = "Figure S1.A Exploring the col.data"}
fplot + geom_bar(aes(type, fill = gender)) + b + y.axis + 
    ggtitle("Gender distribution across normal and tumour samples")
```
As expected, there are more female samples than male samples. As we mentioned before, tiroid cancer is more prevalent in women than men and we can see that not only for tumor samples but for normals samples too. In addition, there are more tumor cases than normal cases.
```{r plots1B, fig.cap = "Figure S1.B Exploring the col.data"}
...
```{r plots1C, fig.cap = "Figure S1.C Exploring the col.data"}
...
```{r plots1D, fig.cap = "Figure S1.D Exploring the col.data"}
...
```{r plots1E, fig.cap = "Figure S1.E Exploring the col.data"}
...
```{r plots1F, fig.cap = "Figure S1.F Exploring the col.data"}
...
```{r plots1H, fig.cap = "Figure S1.H Exploring the col.data"}
...
```{r plots1I, fig.cap = "Figure S1.I Exploring the col.data", fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE}
labels <- c("Other specify", "Papillary - Classical", "Papillary - Follicular", 
            "Papillary - Tall Cell", "NA")
...
thca.filtered$race <- replace(thca.filtered$race, 
                          which(is.na(thca.filtered$race)), "WHITE")
thca.filtered$ethnicity <- replace(thca.filtered$ethnicity, 
                               which(is.na(thca.filtered$ethnicity)), "NOT HISPANIC OR LATINO")
```

We explore again the variables for this subset:
```{r subset.explore, fig.cap = "Figure S2 Exploring filtered data"}
meta.filtered.subset <- filtered_filt[row.names(filtered_filt) %in% sample_names, ]
mplot <- ggplot(meta.filtered.subset) + y.axis
mplot + geom_bar(aes(type, fill = gender)) + b
mplot + geom_bar(aes(type, fill = tumor_status)) + b + 
    scale_fill_discrete(name="tumor status")
...
mplot + facet_wrap(~type)+geom_bar(aes(tumor_focality, fill = gender)) + b + l + xlab("tumor focality")
mplot + geom_bar(aes(histologic_diagnosis, fill = gender)) + b + l + 
    scale_x_discrete(name = "histogic diagnosis", breaks=waiver(), 
    labels = c("Papillary - Classical", "NA")) + 
    ggtitle("Gender distribution across different types of hystologic diagnosis")

ggplot(meta.filtered.subset[!is.na(meta.filtered.subset$age_at_diagnosis), ]) +
...
than the rest. To identify which are these samples we may simply look at the
...
and normal samples separately, and are shown in Figure S4.
...
```{r distRawExp, fig.height = 4, fig.width = 10, out.width = "800px", fig.cap = "Figure S4: Non-parametric density distribution of expression profiles per sample.", message=FALSE}
library(geneplotter)
par(mfrow = c(1, 2))
...
```{r distExp, fig.height = 4, fig.width = 10, out.width = "800px", fig.cap = "Figure S5: Non-parametric density distribution of expression profiles per sample."}
...
Let's calculate now the average expression per gene through all the samples. Figure S6
shows the distribution of those values across genes.

```{r exprdist, out.width = "400px", fig.cap = "Figure S6: Distribution of average expression level per gene."}
par(mfrow = c(1, 2))
...
```{r maPlotsTumor, fig.height = 16, fig.width = 6, dpi = 100, fig.cap = "Figure S7: MA-plots of the tumor samples."}
...
```{r maPlotsNormal, fig.height = 16, fig.width = 6, dpi = 100, fig.cap = "Figure S8: MA-plots of the normal samples."}
par(mfrow = c(8, 3), mar = c(4, 3, 4, 1))
...
```{r clustering.n, fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE, fig.cap = "Figure S9: Hierarchical clustering of the samples."}
...
```{r mds, fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE, fig.cap = "Figure S10: Multidimensional scaling plot of the samples."}
...
#!!!!!!
```{r clustering.combat, message=FALSE, fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE, fig.cap = "Figure S11: Hierarchical clustering of the samples."}
library("sva")
...
```{r clustering.QRdecomposition, fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE, fig.cap = "Figure S12: Hierarchical clustering of the samples."}
library("limma")
...
```{r clustring.svd, fig.height = 7, fig.width = 14, dpi = 100, echo = TRUE, fig.cap = "Figure S13: Hierarchical clustering of the samples."}
library("corpcor")
...
After all the process the best one, that separates better tumor and normal samples is the qr decomposition, which will be used for futher analysis.
...
pv <- f.pvalue(assays(thca.filtered)$qrexp, mod, mod0)
...
```{r pdist, out.width = "500px", fig.cap = "Figure S14: Distribution of raw p-values for an F-test on every gene between tumor and normal samples."}
par(mfrow=c(1,1))
...
Figure S15 shows the resulting distribution of p-values.

```{r psvdist, out.width = "500px", fig.cap = "Figure S15 Distribution of raw p-values for an F-test on every gene between tumor and normal samples, adjusting for surrogate variables estimated with SVA."}
