...
```{r maPlotsTumor, fig.height = 10, fig.width = 6, fig.cap = "\\label{maPlotsTumor}Figure 13.A: MA-plots of the tumor samples.", autodep=TRUE}
...
```{r maPlotsNormal, fig.height = 10, fig.width = 6, fig.cap = "\\label{maPlotsNormal}Figure 13.B: MA-plots of the normal samples.", autodep=TRUE}
par(mfrow = c(8, 3), mar = c(4, 3, 4, 1))
...
Thus the remaining  samples are `r ncol(thca.filtered)` from `r ncol(thca.filtered)/2` patients. 
...
```{r fun.enrich.params_tumors, message = FALSE, autodep = TRUE, warnings = FALSE}
library(GOstats)
...
```{r fun.enrich.fmd, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label{tumor7}Figure 25.H: Representation of the P-value and effect of the GO of the down-regulated genes in both  male and female comparisons", autodep=TRUE}
...
```{r fun.enrich.tfn, echo=TRUE, message=FALSE, fig.cap="\\label{tumor8}Figure 25.I: Representation of the P-value and effect of the GO of the genes with different regulation in the comparison between normal and tumors and females", autodep=TRUE}
