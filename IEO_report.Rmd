...
fplot + facet_wrap(~ gender) + geom_bar(aes(type, fill = ethnicity)) + b + y.axis + ggtitle("... normal and tumour samples")
fplot + facet_wrap(~ ethnicity) + geom_bar(aes(race, fill = gender)) + b + l + y.axis + scale_x_discrete(breaks=waiver(), labels=c("AM. INDIAN OR ALASKA NAT.", "ASIAN", "BLACK OR AFRICAN AM.", "WHITE", "NA")) + ggtitle("Ethnicity and race ...")
...
hd.xaxis = scale_x_discrete(name = "histologic diagnosis", breaks=waiver(), labels=c("Other specify", "Papillary - Classical", "Papillary - Follicular", "Papillary - Tall Cell", "NA"))
fplot + geom_bar(aes(histologic_diagnosis, fill = gender)) + b + l + hd.xaxis + y.axis + ggtitle("Gender distribution across different types of hystologic diagnosis")
ggplot(na.omit(as.data.frame(filtered))) + geom_bar(aes(age_at_diagnosis, fill = histologic_diagnosis)) + b + l +  labs(x="age at diagnosis", y = y.axis)+ scale_fill_discrete(name = "histologic diagnosis", breaks=waiver(), labels = c("Other specify", "Papillary - Classical", "Papillary - Follicular", "Papillary - Tall Cell", "NA")) + ggtitle("Histologic diagnosis distribution across different ages")
```
...
hist(avgexp.cqn, xlab = "log2 CPM", main = "", las = 1)
abline(v = 1, col = "red", lwd = 2)
```
...
To double check if they have some effect on the batch we make the hierarchical clustering of them:
...
outcome <- substr(sample_names, 9, 12)
...
plot.batch <- function(sampleDendrogram, batching, se, out, sample.names = NULL){
    ...
    plot(sampleDendrogram, main = "Hierarchical clustering")
    legend("topright", paste("Batch", levels(factor(batching))), fill = sort(unique(batch)))
}

plot.batch(sampleDendrogram, portion, thca.filtered, outcome, sample_names)
plot.batch(sampleDendrogram, samplevial, thca.filtered, outcome, sample_names)
plot.batch(sampleDendrogram, tss, thca.filtered, outcome, sample_names)
plot.batch(sampleDendrogram, as.character(thca.filtered$type), thca.filtered, outcome, sample_names)
```
...
for (batching in list(portion, as.character(thca.filtered$type), as.character(thca.filtered$gender))){
    ...
    legend("topright", paste("Batch", levels(factor(batching))),
...
```{r sva.model}
library(sva)
variables <- c("tumor_size_width.1", "tumor_size_width.2", "tumor_size_width", "gender", "age_at_diagnosis", "tumor_tissue_site", "histologic_diagnosis", "ajcc_pathologic_tumor_stage","residual_tumor", "type" )
form <- as.formula(paste("~", paste(variables, collapse="*")))
mod <- model.matrix(form, filtered[sample_names,])
mod0 <- model.matrix(~ 1, colData(thca.filtered))
