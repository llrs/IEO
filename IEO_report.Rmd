...
go_analisys <- function(go_obj, annots.GO){
    hgOver <- hyperGTest(go_obj)
    
    GO <- GOstats::summary(hgOver)
    
    GO <- subset(GO, Size >= 5 & Count >= 5 & Pvalue <= 0.05)
    # Visualize the data
...
head(GO_females_males_down$GO)
plot(GO_females_males_down$plotgo)
```

```{r fun.enrich.tmu, echo=TRUE, message=FALSE, warning=FALSE}
GO_tumor_males_up <- go_analisys(go_tumor_males_up, all.Human.GO)
head(GO_tumor_males_up$GO)
plot(GO_tumor_males_up$plotgo)
```


```{r fun.enrich.tmd, echo=TRUE, message=FALSE, warning=FALSE}
GO_tumor_males_down <- go_analisys(go_tumor_males_down, all.Human.GO)
head(GO_tumor_males_down$GO)
plot(GO_tumor_males_down$plotgo)
```

```{r fun.enrich.tfu, echo=TRUE, message=FALSE, warning=FALSE}
GO_tumor_females_up <- go_analisys(go_tumor_females_up, all.Human.GO)
head(GO_tumor_females_up$GO)
plot(GO_tumor_females_up$plotgo)
```

```{r fun.enrich.tfd, echo=TRUE, message=FALSE}
GO_tumor_females_down <- go_analisys(go_tumor_females_down, all.Human.GO)
head(GO_tumor_females_down$GO)
plot(GO_tumor_females_down$plotgo)
```
