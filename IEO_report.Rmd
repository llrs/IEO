...
We want ensembl annotation:
```{r annot}
library("org.Hs.eg.db")
annot <- select(org.Hs.eg.db, keys = rownames(thca.filtered), columns = c("SYMBOL", "ENSEMBL", keytype = "ENTREZID"))
```
...
DE_tumor <- rownames(tt_tumor)[tt_tumor$adj.P.Val < 0.05] 

DE_gender <- rownames(tt_gender)[tt_gender$adj.P.Val < 0.05] 

params_tumor <- new("GOHyperGParams", geneIds=DE_tumor, universeGeneIds=geneUniverse,
                annotation="org.Hs.eg.db", ontology="BP",
                pvalueCutoff=0.05, testDirection="over", conditional = TRUE)

params_gender <- new("GOHyperGParams", geneIds=DE_tumor, universeGeneIds=geneUniverse,
                annotation="org.Hs.eg.db", ontology="BP",
                pvalueCutoff=0.05, testDirection="over", conditional = TRUE)
```
...
library(xtable)
xtab_tumor <- xtable(GO_tumor, align="l|c|r|r|r|r|r|p{3cm}|p{3cm}|c|")
