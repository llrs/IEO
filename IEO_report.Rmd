...
```{r plots1A, fig.cap = fig$cap("Figure S1.A","Exploring the col.data")}
...
```{r plots1B, fig.cap = fig$cap("Figure S1.B","Exploring the col.data")}
...
```{r plots1C, fig.cap = fig$cap("Figure S1.C","Exploring the col.data")}
...
```{r plots1D, fig.cap = fig$cap("Figure S1.D","Exploring the col.data")}
...
```{r plots1E, fig.cap = fig$cap("Figure S1.E","Exploring the col.data")}
fplot + geom_bar(aes(x=gender, fill = race)) + b + y.axis + 
    ggtitle("Gender and ethnicity")
```
...
```{r plots1F, fig.cap = fig$cap("Figure S1.F","Exploring the col.data")}
fplot + facet_wrap(~ type ) + geom_bar(aes(tumor_focality, fill = gender)) + b +
    l + y.axis + xlab("tumor focality")
```
There is no data about tumor focality in the normal samples, because as we could imagine the focality is a feature of the tumor samples.
```{r plots1H, fig.cap = fig$cap("Figure S1.H","Exploring the col.data")}
...
```
Here, we can see that the tumor samples came mainly from classical Papillary Thyroid Carcinoma. We expect differential expression between the different thyroid cancer phenotypes so this might be a source of variability that must be taken into account for the selection of the subset of study.
```{r plots1I, fig.cap = fig$cap("Figure S1.I","Exploring the col.data")}
labels <- c("Other specify", "Papillary - Classical", "Papillary - Follicular", 
            "Papillary - Tall Cell", "NA")
...
ggplot(meta.filtered.subset[!is.na(meta.filtered.subset$age_at_diagnosis), ]) +
    geom_bar(aes(age_at_diagnosis, fill = histologic_diagnosis)) + b + l + 
...
We can confirm that there isn't a clear group of normal and tumors, in fact, some tumoral samples do mix with the normal ones. It seems that the females have more variance between healthy and tumoral.
