...
fplot + facet_wrap(~ ethnicity) + geom_bar(aes(ethnicity, fill = gender)) + b + l + 
...
fplot + geom_bar(aes(x=gender, fill = race)) + b + y.axis + ggtitle("Gender and ethnicity")
```
...
ggplot(filtered[!is.na(filtered$age_at_diagnosis), ]) + geom_bar(aes(age_at_diagnosis, na.rm = T, fill = histologic_diagnosis)) + b + l +  labs(x="age at diagnosis", y = y.axis)+ scale_fill_discrete(name = "histologic diagnosis", breaks=waiver(), labels = c("Other specify", "Papillary - Classical", "Papillary - Follicular", "Papillary - Tall Cell", "NA")) + ggtitle("Histologic diagnosis distribution across different ages")
```
...
thca.filtered$ethnicity <- replace(thca.filtered$ethnicity,which(is.na(thca.filtered$ethnicity)), "NOT HISPANIC OR LATINO")
```
...
ggplot(meta.filtered.subset[!is.na(meta.filtered.subset$age_at_diagnosis), ]) + geom_bar(aes(age_at_diagnosis, fill = histologic_diagnosis)) + b + l + y.axis + xlab("age at diagnosis") + scale_fill_discrete(name = "histologic diagnosis", breaks=waiver(), labels = c("Papillary - Classical", "NA")) + ggtitle("Histologic diagnosis distribution across different ages")
```
