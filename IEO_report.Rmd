...
```
<<<<<<< HEAD
After all this first analysis, we can conclude that aproximatelly 4000 genes have different expression levels among tumor and normal samples. 
=======
...
```{r DE, cache = TRUE}
<<<<<<< HEAD
# Transform the log2 normalized by cqn to counts with the surrogate variables
n_counts <- apply(sv$sv, 1:2, function(x){2^(x)}) 
=======
...
```{r tt, cache = TRUE}
tt_tumor <- topTable(fit2, number = Inf, coef = "NormalVsDisease")
tt_gender <- topTable(fit2, number = Inf, coef = "FemaleVsMale")
tt_females <- topTable(fit2, number = Inf, coef = "FemaleDiseaseVsFemaleNormal")
tt_males <- topTable(fit2, number = Inf, coef = "MaleDiseaseVsMaleNormal")
...
volcanoplot(fit2, coef = "FemaleVsMale", highlight = 7, fit2$genes$symbol, main = "model Gender", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)
volcanoplot(fit2, coef = "NormalVsDisease", highlight = 7, fit2$genes$symbol, main = "model Disease", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)

volcanoplot(fit2, coef = "MaleDiseaseVsMaleNormal", highlight = 7, fit2$genes$symbol, main = "Comparing males", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)

volcanoplot(fit2, coef = "FemaleDiseaseVsFemaleNormal", highlight = 7, fit2$genes$symbol, main = "Comparing Females", las = 1)
abline(h = 4.6) # 99% of confidence
abline(v = 2)
abline(v = -2)
```
...
dim(tt_males[abs(tt_males$logFC) > 2 & tt_males$B > 4.6,])[1]
dim(tt_females[abs(tt_gender$logFC) > 2 & tt_females$B > 4.6,])[1]
```
...
library(GOstats)
=======
We can see the names overlap of the different comparisons
>>>>>>> Correctly removing surrogates
...
DE_males <- unique(tt_males[abs(tt_males$logFC) > 2 & 
                                  tt_males$B > 4.6 & 
                                  tt_males$adj.P.Val < 0.05, "Row.names"])

DE_females <- unique(tt_females[abs(tt_females$logFC) > 2 & 
                                  tt_females$B > 4.6 & 
                                  tt_females$adj.P.Val < 0.05, "Row.names"])
...
```{r fun.enrich.tumor_tumors, cache = TRUE}
...
```{r fun.enrich.gender_tumors, eval=FALSE, include=FALSE}
