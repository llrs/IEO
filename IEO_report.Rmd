...
qrexp <- removeBatchEffect(assays(thca.filtered)$cqn.logCPM, as.numeric(as.factor(tss)), 
                           design = mod)
...
pvsv <- f.pvalue(assays(thca.filtered)$qrexpr, modsv, mod0sv)
...
cqn.n <- apply(assays(thca.filtered)$qrexpr, 1:2, function(x){2^(x)}) 
