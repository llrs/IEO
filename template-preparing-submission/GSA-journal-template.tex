...
We can see from the QQplots that the DE genes from the comparison Disease vs. Normal differ from adjusting to a T-Student distribution (null hypothesis/no-DE) more than the gender comparison (Males vs Females). We can see it also reflected within genders in the Disease-Female vs. Normal-Female and Disease-Male vs. Normal-Male. If we have a look to the volcano plots (Figure...) we can also see that the DE genes within genders are more due to the disease itself rather then a gender effect. In other words, as expected "type" is a much better predictor of DE genes than "gender" factor. Concretely, there are 1260 DE genes between Normal and Tumor/Disease samples, 2 DE genes between Female and Male samples, 288 DE genes between Male-Normal vs. Male-Disease and 371 DE genes between Female-Normal vs. Female-Disease. From those DE genes of the different contrasts, we want to see how many of them are also DE between them. To do so, we can have a look at Figure... that we have created with the R package Vennerable \citep{Venn}. 

From the DE genes that we find between Disease and Normal only 11 are found to be also differentially expressed in females (Female-Normal vs. Female-Disease) and 5 in males (Male-Normal vs. Male-Disease). So, even though the pattern from the volcano plots and the QQ plots of the 3 comparisons were similar actually a small amount of DE genes are shared to be differentially expressed among them. In contrast, from the lists of DE genes of each gender the majority of them are shared (249). 

From the 5 DE genes of the venn diagram between male and tumor only 3 are down-regulated in both DE gene sets and 2 differ in how they are differentially expressed. A similar situation happens when looking at females where aproximately half of them differ in their regulation between female and tumor sets, only 4 out of 11 are down regulated in both sets and 1 out of 11 is up regulated. 
